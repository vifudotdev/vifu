...
impl ApiKeyRow {
    fn into_record(self, profile_ids: Vec<Uuid>) -> Result<ApiKeyRecord, ApiError> {
        let row = self;
        let agent_scope = match row.scope_mode.as_str() {
            "all" if profile_ids.is_empty() => ApiKeyAgentScope::All,
            "selected" => ApiKeyAgentScope::Selected { profile_ids },
...
#[derive(Debug, FromRow)]
struct ProjectRuntimeChannelRow {
    id: Uuid,
    project_id: Uuid,
    name: String,
    public_id: Uuid,
    launch_key_prefix: String,
    allowed_origins: Json<Vec<String>>,
    enabled: bool,
    created_at: DateTime<Utc>,
    updated_at: DateTime<Utc>,
}

impl From<ProjectRuntimeChannelRow> for ProjectRuntimeChannel {
    fn from(row: ProjectRuntimeChannelRow) -> Self {
        Self {
            id: row.id,
            project_id: row.project_id,
            name: row.name,
            public_id: row.public_id,
            launch_key_prefix: row.launch_key_prefix,
            allowed_origins: row.allowed_origins.0,
            enabled: row.enabled,
            created_at: row.created_at,
...
impl From<ProviderConnectionRow> for ProviderConnection {
    fn from(row: ProviderConnectionRow) -> Self {
        Self {
            id: row.id,
            project_id: row.project_id,
...
#[derive(Debug, FromRow)]
struct ProviderConnectionSecretRow {
    id: Uuid,
    project_id: Uuid,
    provider_key: String,
    source_kind: String,
...
impl From<ProviderConnectionSecretRow> for ProviderConnectionSecret {
    fn from(row: ProviderConnectionSecretRow) -> Self {
        Self {
            id: row.id,
            project_id: row.project_id,
            provider_key: row.provider_key,
...
pub async fn ready(pool: &SqlitePool) -> Result<(), ApiError> {
    sqlx::query("SELECT 1").execute(pool).await?;
    Ok(())
}

pub async fn mark_agent_gateway_sessions_disconnected(pool: &SqlitePool) -> Result<(), ApiError> {
    sqlx::query(
        "UPDATE agent_gateway_sessions SET
            status = 'disconnected', last_seen_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), disconnected_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
...
pub async fn promote_runtime_deployment(
    ...
    sqlx::query("UPDATE runtime_deployments SET is_primary = 0 WHERE project_id = $1")
        .bind(project_id)
    ...
    .await?
    {
        sqlx::query(
            "UPDATE projects
             SET gateway_id = $2, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
             WHERE id = $1",
        )
        .bind(project_id)
        .bind(gateway_id)
...
pub async fn delete_runtime_deployment(
    ...
    let result = sqlx::query(
        "DELETE FROM runtime_deployments
         WHERE project_id = $1 AND name = $2 AND is_primary = 0",
    )
    .bind(project_id)
    .bind(name)
    .execute(pool)
        ...
        "DELETE FROM runtime_deployment_gateways
         WHERE gateway_id = $2
           ...
           AND deployment_id IN (
             SELECT id FROM runtime_deployments WHERE project_id = $1
           )",
    )
    .bind(project_id)
    .bind(gateway_id)
        ...
        sqlx::query(
            "UPDATE projects
             SET gateway_id = COALESCE((
                   ...
                   WHERE deployment.project_id = $1 AND deployment.is_primary = 1
...
pub async fn unassign_runtime_deployment_gateway(
    ...
    let result = sqlx::query(
        "DELETE FROM runtime_deployment_gateways
         WHERE deployment_id = $1 AND gateway_id = $3
           AND EXISTS (
    ...
    sqlx::query_scalar(
        "SELECT EXISTS(
           ...
           WHERE deployment.project_id = $1
             ...
             AND deployment.remote_invocation_enabled = 1
         )",
    )
    .bind(project_id)
    .bind(gateway_id)
...
pub async fn authorize_runtime_distribution_gateway(
    ...
    sqlx::query(
        "INSERT INTO runtime_distribution_gateways(distribution_id, machine_id, gateway_id)
         SELECT $1, $2, $3
         WHERE (SELECT COUNT(*) FROM runtime_distribution_gateways
                WHERE distribution_id = $1) < $4
         ON CONFLICT(distribution_id, machine_id) DO NOTHING",
    )
    .bind(distribution_id)
    .bind(machine_id)
    ...
    .await
    .map_err(map_database_error)?;
    let gateway_id = sqlx::query_scalar::<_, String>(
    ...
    .fetch_optional(&mut *transaction)
    .await?
    .ok_or_else(|| ApiError::Conflict("runtime distribution device limit reached".to_string()))?;
    sqlx::query(
        "INSERT INTO runtime_deployment_gateways(deployment_id, gateway_id)
         VALUES ($1, $2) ON CONFLICT DO NOTHING",
    )
...
pub async fn promote_guest_project(pool: &SqlitePool, project_id: Uuid) -> Result<bool, ApiError> {
    let mut transaction = pool.begin().await?;
...
pub async fn get_active_guest_project_for_gateway(
    ...
    let guest = sqlx::query_as::<_, (Uuid, DateTime<Utc>)>(
        "SELECT project_id, expires_at
         FROM guest_projects
         WHERE gateway_id = $1 AND claimed_at IS NULL
...
pub async fn get_active_guest_project_by_project_id(
    ...
    sqlx::query_scalar(
        "SELECT expires_at
         FROM guest_projects
         WHERE project_id = $1 AND claimed_at IS NULL
           AND expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now')",
    )
    .bind(project_id)
    .fetch_optional(pool)
...
pub async fn count_active_guest_projects(pool: &SqlitePool) -> Result<i64, ApiError> {
    sqlx::query_scalar(
        "SELECT COUNT(*) FROM guest_projects
         WHERE claimed_at IS NULL AND expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now')",
    )
    .fetch_one(pool)
    .await
    .map_err(ApiError::Database)
...
pub async fn prune_expired_guest_projects(pool: &SqlitePool) -> Result<u64, ApiError> {
    let result = sqlx::query(
...
pub async fn create_project_runtime_channel(
    ...
    let row = sqlx::query_as::<_, ProjectRuntimeChannelRow>(
    ...
    .bind(Json(input.allowed_origins))
    .fetch_one(pool)
    .await
    .map_err(map_database_error)?;
    Ok(row.into())
...
pub async fn list_project_runtime_channels(
    ...
    let rows = sqlx::query_as::<_, ProjectRuntimeChannelRow>(
    ...
    .await
    .map_err(ApiError::Database)?;
    Ok(rows.into_iter().map(ProjectRuntimeChannel::from).collect())
}
...
pub async fn active_runtime_launch_project(
    ...
    sqlx::query_scalar(
        "SELECT project_id
         ...
         WHERE token_hash = $1 AND expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now')",
    )
    .bind(token_hash)
    .fetch_optional(pool)
    .await
...
pub async fn create_project(
    ...
    sqlx::query(
        "INSERT INTO runtime_deployments(
            id, project_id, name, is_primary, config_sync_enabled, trace_mode,
            remote_invocation_enabled
         ) VALUES ($1, $2, 'development', 1, 1, 'summary', 0)",
    )
    .bind(deployment_id)
    .bind(project.id)
    .execute(&mut *transaction)
...
pub async fn delete_project(pool: &SqlitePool, id: Uuid) -> Result<(), ApiError> {
...
pub async fn get_provider_connection_secret(
    ...
    let row = sqlx::query_as::<_, ProviderConnectionSecretRow>(
        "SELECT id, project_id, provider_key, source_kind, source_key,
                name, provider_type, base_url, config,
                encrypted_secret_json, secret_keys, display_secret, status,
...
pub async fn update_provider_connection_status(
    ...
    let row = sqlx::query_as::<_, ProviderConnectionRow>(
        "UPDATE provider_connections SET status = $2, last_checked_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
         WHERE id = $1
         RETURNING id, project_id, provider_key, source_kind, source_key,
                   name, provider_type, base_url, config,
                   secret_keys, display_secret, status, last_checked_at, created_at, updated_at",
    ...
    sqlx::query_as::<_, crate::models::ArchivedProjectAgentSource>(
        "SELECT profile.id AS profile_id,
                ...
                COALESCE(
                    ...
                    (
                        SELECT capability.provider_key
                        FROM agent_profile_capabilities AS capability
                        WHERE capability.profile_version_id = version.id
                ...
                COALESCE(
                    binding.provider,
                    (
                        SELECT capability.provider_type
                        FROM agent_profile_capabilities AS capability
                        WHERE capability.profile_version_id = version.id
                        ORDER BY capability.created_at ASC
                        LIMIT 1
...
pub async fn restore_project_profile(
    ...
    let profile = sqlx::query_as::<_, AgentProfile>(
        "UPDATE agent_profiles
         SET archived_at = NULL, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
         ...
         WHERE profile.project_id = $1
           ...
           AND EXISTS (
               ...
               WHERE binding.profile_id = profile.id
                 ...
                 AND json_extract(version.source, '$.managed') = 1
           )",
    )
    .bind(project_id)
...
pub async fn refresh_discovered_binding_record(
    ...
    sqlx::query(
        "UPDATE agent_bindings
         SET gateway_id = $2,
             config = json_set(config, '$.agentName', $3),
             updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
         WHERE id = $1",
    )
    .bind(binding_id)
...
async fn project_binding_ids(pool: &SqlitePool, project_id: Uuid) -> Result<Vec<Uuid>, ApiError> {
    sqlx::query_scalar(
...
async fn validate_project_bindings(
    ...
    let mut query =
        QueryBuilder::<Sqlite>::new("SELECT COUNT(*) FROM agent_bindings WHERE id IN (");
    let mut ids = query.separated(", ");
    for binding_id in binding_ids {
        ids.push_bind(*binding_id);
    }
...
pub async fn list_profiles(pool: &SqlitePool) -> Result<Vec<AgentProfile>, ApiError> {
...
pub async fn get_profile(pool: &SqlitePool, id: Uuid) -> Result<AgentProfile, ApiError> {
    sqlx::query_as::<_, AgentProfile>(
        "SELECT id, project_id, slug, name, description, active_version_id, archived_at,
                created_at, updated_at
...
pub async fn delete_profile(pool: &SqlitePool, id: Uuid) -> Result<(), ApiError> {
    let result = sqlx::query(
        "UPDATE agent_profiles
         SET archived_at = COALESCE(archived_at, strftime('%Y-%m-%dT%H:%M:%fZ', 'now')), updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
         WHERE id = $1",
    )
    .bind(id)
    .execute(pool)
...
pub async fn archive_project_profile(
    ...
    sqlx::query(
        "UPDATE agent_endpoints
         SET enabled = FALSE, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
         WHERE profile_id = $1",
    )
    .bind(profile_id)
    .execute(&mut *transaction)
    ...
    sqlx::query(
        "DELETE FROM project_bindings
         WHERE project_id = $1
           AND binding_id IN (
               SELECT id FROM agent_bindings WHERE profile_id = $2
           )",
    )
    .bind(project_id)
    .bind(profile_id)
...
pub async fn create_profile_version(
    ...
    let active_version_id = sqlx::query_scalar::<_, Option<Uuid>>(
        "SELECT active_version_id FROM agent_profiles WHERE id = $1",
    )
    .bind(profile_id)
    .fetch_optional(&mut *transaction)
    .await?
    ...
    if active_version_id.is_none() {
        sqlx::query(
            "UPDATE agent_profiles
             SET active_version_id = $2, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
             WHERE id = $1",
        )
        .bind(profile_id)
        .bind(version_id)
...
pub async fn set_profile_rollout(
    ...
    query.push_bind(profile_id).push(" AND id IN (");
    let mut ids = query.separated(", ");
    for version_id in &version_ids {
        ids.push_bind(*version_id);
    }
    ids.push_unseparated(") AND archived_at IS NULL");
    let versions = query
        .build_query_as::<AgentProfileVersion>()
        .fetch_all(pool)
        .await?;
...
pub async fn list_bindings(pool: &SqlitePool) -> Result<Vec<AgentBinding>, ApiError> {
...
pub async fn get_binding(pool: &SqlitePool, id: Uuid) -> Result<AgentBinding, ApiError> {
    sqlx::query_as::<_, AgentBinding>(
        "SELECT id, profile_id, provider, gateway_id, agent_id, config, created_at, updated_at
...
pub async fn delete_binding(pool: &SqlitePool, id: Uuid) -> Result<(), ApiError> {
    delete_by_id(pool, "agent_bindings", id).await
}

pub async fn list_endpoints(pool: &SqlitePool) -> Result<Vec<AgentEndpoint>, ApiError> {
...
pub async fn list_enabled_endpoints(pool: &SqlitePool) -> Result<Vec<AgentEndpoint>, ApiError> {
    sqlx::query_as::<_, AgentEndpoint>(
        "SELECT id, slug, name, profile_id, binding_id, enabled, request_timeout_ms,
                created_at, updated_at
...
pub async fn get_endpoint(pool: &SqlitePool, id: Uuid) -> Result<AgentEndpoint, ApiError> {
    sqlx::query_as::<_, AgentEndpoint>(
        "SELECT id, slug, name, profile_id, binding_id, enabled, request_timeout_ms,
                created_at, updated_at
...
pub async fn delete_endpoint(pool: &SqlitePool, id: Uuid) -> Result<(), ApiError> {
...
pub async fn list_api_keys(pool: &SqlitePool) -> Result<Vec<ApiKeyRecord>, ApiError> {
    let rows = sqlx::query_as::<_, ApiKeyRow>(
        "SELECT id, project_id, name, scope_mode, permissions, key_prefix,
    ...
    .fetch_all(pool)
    .await?;
    let scope_rows = sqlx::query_as::<_, (Uuid, Uuid)>(
        "SELECT api_key_id, profile_id
         FROM api_key_profile_scopes
         ORDER BY api_key_id, profile_id",
    )
    .fetch_all(pool)
    .await?;
    let mut scopes = HashMap::<Uuid, Vec<Uuid>>::new();
    for (api_key_id, profile_id) in scope_rows {
        scopes.entry(api_key_id).or_default().push(profile_id);
    }
    rows.into_iter()
        .map(|row| {
            let profile_ids = scopes.remove(&row.id).unwrap_or_default();
            row.into_record(profile_ids)
        })
        .collect()
}
...
pub async fn get_api_key(pool: &SqlitePool, id: Uuid) -> Result<ApiKeyRecord, ApiError> {
...
pub async fn revoke_api_key(pool: &SqlitePool, id: Uuid) -> Result<ApiKeyRecord, ApiError> {
    let updated = sqlx::query_scalar::<_, Uuid>(
        "UPDATE api_keys SET revoked_at = COALESCE(revoked_at, strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) WHERE id = $1 RETURNING id",
    )
    .bind(id)
    .fetch_optional(pool)
    .await?
...
pub async fn delete_api_key(pool: &SqlitePool, id: Uuid) -> Result<(), ApiError> {
...
pub async fn create_realtime_session(
    ...
    sqlx::query(
        "DELETE FROM realtime_sessions WHERE expires_at <= strftime('%Y-%m-%dT%H:%M:%fZ', 'now')",
    )
    .execute(pool)
    .await?;
...
pub async fn active_realtime_session_by_hash(
    ...
    sqlx::query_as::<_, RealtimeSession>(
        "SELECT id, project_id, profile_id, api_key_id, expires_at, created_at
         ...
         WHERE project_id = $1 AND token_hash = $2 AND expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now')",
    )
    .bind(project_id)
    .bind(token_hash)
    .fetch_optional(pool)
...
pub async fn active_api_key_by_hash_optional(
    ...
    .await?;
    match row {
        Some(row) => Ok(Some(api_key_record(pool, row).await?)),
        None => Ok(None),
    }
}

async fn api_key_record(pool: &SqlitePool, row: ApiKeyRow) -> Result<ApiKeyRecord, ApiError> {
...
async fn validate_api_key_agent_scope(
    ...
    let mut query =
        QueryBuilder::<Sqlite>::new("SELECT COUNT(*) FROM agent_profiles WHERE project_id = ");
    query.push_bind(project_id).push(" AND id IN (");
    let mut ids = query.separated(", ");
    for profile_id in profile_ids {
        ids.push_bind(*profile_id);
    }
    ids.push_unseparated(") AND archived_at IS NULL");
    let count = query.build_query_scalar::<i64>().fetch_one(pool).await?;
...
pub async fn resolve_project_model_route(
    ...
    let endpoint_id = Uuid::parse_str(model).ok();
        ...
        "SELECT endpoint.id AS endpoint_id, endpoint.slug AS endpoint_slug,
         ...
         ORDER BY
           CASE
             ...
             WHEN endpoint.id = $3 THEN 1
             WHEN profile.slug = $2 THEN 2
             WHEN binding.agent_id = $2 THEN 3
             ELSE 4
           END,
    ...
    sqlx::query_as::<_, AgentGatewayAuthorization>(
        "UPDATE agent_gateway_authorizations
         SET previous_token_hash = CASE WHEN status = 'active' THEN token_hash ELSE NULL END,
             previous_token_expires_at = CASE WHEN status = 'active' THEN MIN(token_expires_at, strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+10 minutes')) ELSE NULL END,
             token_prefix = $2,
             token_hash = $3,
...
pub async fn authenticate_agent_gateway_device_token(
    ...
    sqlx::query_scalar::<_, String>(
        "UPDATE agent_gateway_authorizations
         ...
         WHERE status = 'active'
           AND (
             (token_hash = $1 AND token_expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))
             OR (previous_token_hash = $1 AND previous_token_expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))
           )
...
pub async fn consume_agent_gateway_machine_enrollment(
    ...
    let claimed = sqlx::query_as::<_, AgentGatewayMachineEnrollmentSecret>(
        "UPDATE agent_gateway_enrollments
         SET gateway_id = $2, consumed_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
        ...
        None => {
            let existing = sqlx::query_as::<_, AgentGatewayMachineEnrollmentSecret>(
                "SELECT id, project_id, deployment_id, owner_user_id, gateway_id, consumed_at,
                        revoked_at, expires_at
                 FROM agent_gateway_enrollments WHERE token_hash = $1",
            )
            .bind(token_hash)
            .fetch_optional(&mut *transaction)
            .await?
    ...
    sqlx::query(
        "UPDATE projects
         SET gateway_id = $1, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
         WHERE id = $2 AND EXISTS(
           SELECT 1 FROM runtime_deployments WHERE id = $3 AND project_id = $2 AND is_primary = 1
         )",
    )
    .bind(gateway_id)
    .bind(enrollment.project_id)
...
pub async fn create_or_get_agent_gateway_pairing(
    ...
    let mut transaction = pool.begin().await?;
    sqlx::query(
        "UPDATE agent_gateway_machines SET last_seen_at = last_seen_at WHERE machine_id = $1",
    )
    .bind(machine_id)
    .execute(&mut *transaction)
    .await?;
...
pub async fn consume_agent_gateway_pairing(
    ...
    sqlx::query_as::<_, AgentGatewayPairingRequest>(
        "UPDATE agent_gateway_pairing_requests
         SET status = 'consumed'
         WHERE id = $1 AND machine_id = $2 AND status = 'approved'
           AND expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
         RETURNING id, machine_id, status, owner_user_id, expires_at, created_at, resolved_at",
    )
    .bind(id)
        ...
        "UPDATE agent_gateway_pairing_requests
         ...
         WHERE id = (
             ...
             WHERE machine_id = $1 AND status = 'approved'
               AND expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
             ORDER BY COALESCE(resolved_at, created_at) ASC, created_at ASC
             LIMIT 1
         )
         RETURNING id, machine_id, status, owner_user_id, expires_at, created_at, resolved_at",
    )
...
pub async fn consume_agent_gateway_enrollment(
    ...
    .execute(&mut *transaction)
    .await?;
    let deployment_primary = sqlx::query_scalar::<_, bool>(
        "SELECT is_primary FROM runtime_deployments
         WHERE id = $1 AND project_id = $2",
    )
    .bind(enrollment.deployment_id)
    .bind(enrollment.project_id)
    ...
    if let Some(session_id) = resume_session_id {
        let updated = sqlx::query_scalar::<_, Uuid>(
            "UPDATE agent_gateway_sessions SET
                ...
                connected_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), last_seen_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), disconnected_at = NULL
...
pub async fn touch_agent_gateway_session(
    pool: &SqlitePool,
    session_id: Uuid,
) -> Result<(), ApiError> {
    sqlx::query(
        "UPDATE agent_gateway_sessions SET last_seen_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), status = 'connected'
         WHERE session_id = $1",
    )
    .bind(session_id)
    .execute(pool)
...
pub async fn close_agent_gateway_session(
    pool: &SqlitePool,
    session_id: Uuid,
) -> Result<(), ApiError> {
    sqlx::query(
...
pub async fn list_agent_gateway_sessions_for_gateways(
    ...
    let mut query = QueryBuilder::<Sqlite>::new(
        "SELECT id, gateway_id, session_id, status, agents, metadata,
                connected_at, last_seen_at, disconnected_at
         FROM agent_gateway_sessions WHERE gateway_id IN (",
    );
    {
        let mut separated = query.separated(", ");
        for gateway_id in gateway_ids {
            separated.push_bind(gateway_id);
        }
    }
    query.push(") ORDER BY connected_at DESC LIMIT 200");
    query
        .build_query_as::<AgentGatewaySession>()
        .fetch_all(pool)
        .await
        .map_err(ApiError::from)
}

pub async fn list_available_agents(pool: &SqlitePool) -> Result<Vec<AvailableAgent>, ApiError> {
    Ok(super::available_agents_from_sessions(
        list_agent_gateway_sessions(pool).await?,
    ))
}

pub async fn create_trace(pool: &SqlitePool, trace: NewTrace<'_>) -> Result<Uuid, ApiError> {
    ...
    let result = sqlx::query(
        "INSERT INTO trace_spans
         ...
         WHERE trace_spans.trace_id = excluded.trace_id
           AND trace_spans.parent_span_id IS excluded.parent_span_id
...
pub async fn get_runtime_trace_target(
    ...
    sqlx::query_as::<_, RuntimeTraceTarget>(
        "SELECT trace.id AS trace_id,
         ...
         LEFT JOIN trace_spans root ON root.id = (
             SELECT span.id
...
pub async fn merge_trace_runtime_generation(
    ...
    sqlx::query(
        "UPDATE trace_spans
         SET completion_start_ms = COALESCE($2, completion_start_ms),
             usage = CASE
                 WHEN $3 IS NULL AND $4 IS NULL THEN usage
                 WHEN $3 IS NOT NULL AND $4 IS NOT NULL THEN
                     json_set(COALESCE(usage, '{}'), '$.inputTokens', $3, '$.outputTokens', $4)
                 WHEN $3 IS NOT NULL THEN
                     json_set(COALESCE(usage, '{}'), '$.inputTokens', $3)
                 ELSE json_set(COALESCE(usage, '{}'), '$.outputTokens', $4)
             END
         WHERE id = (
             SELECT span.id FROM trace_spans span
             JOIN endpoint_traces trace ON trace.id = span.trace_id
    ...
    sqlx::query(
        "UPDATE trace_spans
         SET input_summary = COALESCE($2, input_summary),
             ...
             attributes = json_patch(COALESCE(attributes, '{}'), $4)
         WHERE id = (
             SELECT span.id FROM trace_spans span
             JOIN endpoint_traces trace ON trace.id = span.trace_id
             WHERE trace.request_id = $1 AND span.parent_span_id IS NULL
...
pub async fn list_trace_scores(
    pool: &SqlitePool,
    trace_id: Uuid,
) -> Result<Vec<TraceScore>, ApiError> {
    sqlx::query_as::<_, TraceScore>(
    ...
    sqlx::query(
        "UPDATE trace_spans
         SET status = $2, duration_ms = $3,
             output_summary = CASE
                 WHEN json_extract(attributes, '$._vifuTraceIo.outputCanonical') = 1
                     THEN output_summary
                 ELSE $4
             END,
             error = $5,
...
pub async fn complete_trace(
    ...
    sqlx::query(
        "UPDATE endpoint_traces SET status = $2, latency_ms = $3, response = $4,
                error = $5, completed_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
         WHERE request_id = $1",
    )
    .bind(request_id)
    .bind(status)
...
pub async fn list_traces(
    ...
    let mut query = QueryBuilder::<Sqlite>::new(
        "SELECT trace.id, trace.request_id, trace.endpoint_id,
                ...
                COALESCE(json_extract(gateway_session.metadata, '$.name'), gateway_session.gateway_id) AS gateway_name,
                ...
                CASE
                    WHEN EXISTS (
                        ...
                        WHERE score.trace_id = trace.id AND score.source = 'application'
                              ...
                              AND json_extract(score.value, '$') = 'fail'
                    ) THEN 'fail'
                    WHEN EXISTS (
                        SELECT 1 FROM trace_scores score
                        WHERE score.trace_id = trace.id AND score.source = 'application'
                              AND score.name IN ('OUTPUT_ACCEPTED', 'ACTION_APPLIED', 'FRAME_PRESENTED')
                              AND json_extract(score.value, '$') = 'unknown'
                    ) THEN 'unknown'
                    WHEN (
                        ...
                        WHERE score.trace_id = trace.id AND score.source = 'application'
                              AND score.name IN ('OUTPUT_ACCEPTED', 'ACTION_APPLIED', 'FRAME_PRESENTED')
                              AND json_extract(score.value, '$') = 'pass'
                    ) THEN 'pass'
                    WHEN (
                        SELECT COUNT(DISTINCT score.name) FROM trace_scores score
                        WHERE score.trace_id = trace.id AND score.source = 'application'
                        ...
                        WHERE score.trace_id = trace.id AND score.source = 'application'
                              AND score.name IN ('OUTPUT_ACCEPTED', 'ACTION_APPLIED', 'FRAME_PRESENTED')
                              AND json_extract(score.value, '$') <> 'notApplicable'
                    ) THEN 'notApplicable'
                    WHEN EXISTS (
                        SELECT 1 FROM trace_scores score
                        WHERE score.trace_id = trace.id AND score.source = 'application'
...
pub async fn get_trace_project_id(
    pool: &SqlitePool,
    trace_id: Uuid,
) -> Result<Option<Uuid>, ApiError> {
...
pub async fn get_trace_identity(
    pool: &SqlitePool,
    trace_id: Uuid,
) -> Result<TraceIdentity, ApiError> {
    let (project_id, profile_id) =
        sqlx::query_as("SELECT project_id, profile_id FROM endpoint_traces WHERE id = ?")
            .bind(trace_id)
            .fetch_optional(pool)
            .await?
            .ok_or(ApiError::NotFound)?;
