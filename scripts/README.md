...
| `dev-connector.sh` | Load `.env.local` or `.env` and run the connector |
...
| `run-self-hosted-e2e.sh` | Exercise connector concurrency, restart, and persistence; accepts `VIFU_E2E_ENV_FILE` |
