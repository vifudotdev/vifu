...
- `packages/console` contains host-neutral `@vifu/console` App, Agent, Provider,
...
`/v1/chat/completions`; the App key selects the App. App CRUD,
traces, keys, and deployment operations use the host's injected request adapter
and retain their App-addressed management routes.
