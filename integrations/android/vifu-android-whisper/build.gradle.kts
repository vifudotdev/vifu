...
val sharedKotlin = rootProject.layout.projectDirectory.dir("whisper-shared/src/main/kotlin")

val generateVifuAndroidWhisper by tasks.registering(Exec::class) {
    group = "build"
    description = "Builds the arm64 Vifu Whisper provider and Kotlin bindings."
    inputs.files(
    ...
    environment("CARGO_TARGET_DIR", repositoryRoot.resolve("target/vifu-android-whisper").absolutePath)
    commandLine(
...
android {
    namespace = "dev.vifu.android.whisper"
...
tasks.named("preBuild").configure { dependsOn(generateVifuAndroidWhisper) }
...
mavenPublishing {
    ...
    coordinates(project.group.toString(), "vifu-android-whisper", project.version.toString())
    publishToMavenCentral(automaticRelease = true)
    if (providers.gradleProperty("signingInMemoryKey").isPresent) signAllPublications()
    pom {
        name.set("Vifu Android Whisper")
        description.set("Optional local whisper.cpp transcription provider for Vifu Android.")
