...
val generateVifuAndroidLlama by tasks.registering(Exec::class) {
    group = "build"
    description = "Builds the optimized arm64 Vifu llama provider and Kotlin bindings."
    inputs.files(
    ...
    environment("CARGO_TARGET_DIR", repositoryRoot.resolve("target/vifu-android-llama").absolutePath)
    commandLine(
...
android {
    namespace = "dev.vifu.android.llama"
...
tasks.named("preBuild").configure { dependsOn(generateVifuAndroidLlama) }
...
mavenPublishing {
    ...
    coordinates(project.group.toString(), "vifu-android-llama", project.version.toString())
    publishToMavenCentral(automaticRelease = true)
    if (providers.gradleProperty("signingInMemoryKey").isPresent) signAllPublications()
    pom {
        name.set("Vifu Android Llama")
        description.set("Optional runtime-selected ARM llama.cpp provider for Vifu Android.")
