...
val sharedKotlin = rootProject.layout.projectDirectory.dir("core-shared/src/main/kotlin")

val generateVifuAndroidCore by tasks.registering(Exec::class) {
    group = "build"
    description = "Builds the arm64 Vifu core runtime and Kotlin bindings."
    inputs.files(
    ...
    environment("CARGO_TARGET_DIR", repositoryRoot.resolve("target/vifu-android-core").absolutePath)
    commandLine(
...
android {
    namespace = "dev.vifu.android.core"
...
tasks.named("preBuild").configure { dependsOn(generateVifuAndroidCore) }
...
mavenPublishing {
    ...
    coordinates(project.group.toString(), "vifu-android-core", project.version.toString())
    publishToMavenCentral(automaticRelease = true)
    if (providers.gradleProperty("signingInMemoryKey").isPresent) signAllPublications()
    pom {
        name.set("Vifu Android Core")
        description.set("Embeddable Vifu Agent Runtime and Gateway for Android.")
